...
const resultPath = verifyReportFile('verify_module_sidebar_phone_display_mode.json');
...
async function readTargetRow(page, text) {
    ...
    return page.evaluate(targetText => {
        ...
        const phoneBadge = row.querySelector('.duty-summary-phone-badge');
        return {
            found: true,
            ...
            hasPhoneLine: !!phone,
            hasPhoneBadge: !!phoneBadge,
            phoneBadgeText: phoneBadge ? phoneBadge.innerText.trim() : ''
        };
    }, text);
}
...
async function run() {
    ...
    const moduleId = `verify_sidebar_phone_mode_${suffix}`;
    const shiftId = `verify_sidebar_phone_shift_${suffix}`;
    const doctorId = `verify_sidebar_phone_doc_${suffix}`;
    const sidebarLabel = `电话模式模块${suffix}`;
    const doctorName = `电话验证医生${suffix}`;

    try {
        const nextData = JSON.parse(JSON.stringify(originalData));
        nextData.modules.push({
            id: moduleId,
            doctorLabel: `电话模式模块${suffix}`,
            groupName: `电话模式班${suffix}`,
            clearLabel: `清空电话模式班${suffix}`,
        ...
        try {
            ...
            separateLineResult = await readTargetRow(page, sidebarLabel);

            const inlineData = await fetchData(headers);
            const inlineModule = inlineData.modules.find(module => module.id === moduleId);
            inlineModule.sidebarPhoneMode = 'inline_after_name';
            await saveData(headers, inlineData);
            ...
            badgeModule.sidebarPhoneMode = 'badge_after_name';
        ...
        const result = {
            success: separateLineResult.found
                && separateLineResult.hasPhoneLine === true
                && separateLineResult.phoneText.includes('13800138000')
                && separateLineResult.namesText.includes(`${doctorName}（主治医师）`)
                && inlineResult.found
                && inlineResult.hasPhoneLine === false
                && inlineResult.hasPhoneBadge === false
                && inlineResult.namesText.includes(`${doctorName}（主治医师，13800138000）`)
                && badgeResult.found
                && badgeResult.hasPhoneLine === false
                && badgeResult.hasPhoneBadge === true
                && badgeResult.phoneBadgeText === '13800138000'
